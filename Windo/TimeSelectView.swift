...
class TimeSelectView: UIView {
    ...
    convenience init(timeDelegate: TimeViewDelegate, timeDate: NSDate){
        self.init(frame: CGRectZero)
        date = timeDate
        timeViewDelegate = timeDelegate
    }
    ...
    func configureTimes(){
        ...
        for time in times {
            ...
            time.delegate = timeViewDelegate
            time.time = timeCount
            timeCount += 1
        }
    }
